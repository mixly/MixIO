...
void jsiInit(bool autoLoad) {
	...
	//jsiConsolePrintf("a = %d, c = %d\n",autoLoad,jshFlashContainsCode());
...
void jsiHandleNewLine(bool execute) {
	if (jsiAtEndOfInputLine()) { // at EOL so we need to figure out if we can execute or not
		if (execute && jsiCountBracketsInInput()<=0) { // actually execute!
			...
			inputCursorPos = 0;
			//jsiConsolePrint("\nline = ");
			//jsiConsolePrintStringVarWithNewLineChar(lineToExecute, 0, 0);
			//jsiConsolePrint("\n");
			// execute!
			...
			//jsiConsolePrint("handle line over! ");
		} else {
...
void jsiHandleChar(char ch) {
	...
	if (ch == 0) {
		...
		if (ch == 79)
			...
			//jsiConsolePrint("10-01\n");
			jsiHandleNewLine(false);
			//jsiConsolePrint("10-02\n");
		}
    ...
    question should be executed without echo */
		...
		if (ch == 0x08 || ch == 0x7F /*delete*/) {
			...
			if (jsiShowInputLine()) {
				//jsiConsolePrint("ok\n");
				//jsiConsolePrintStringVarUntilEOL(inputLine, 0, 0xFFFFFFFF, true/*and backup*/);
				//jsiConsolePrint("\n");
				jsiConsolePrint(strToAppend);
			}
		}
	}
	//jsiConsolePrint("end for handle char !\n");
}
...
void jsiIdle() {
	...
	//if(first >= 1 && first <= 10)  jsiConsolePrintf("maxEvents = %d  %d\n",maxEvents,jshHasEvents());
	//jsiConsolePrintf("maxEvents = %d  %d\n",maxEvents,jshHasEvents());
	while (maxEvents-- && jshPopIOEvent(&event)) {
		...
		//jsiConsolePrint("in while loop\n");
		loopsIdling = 0; // because we're not idling
		if (eventType == consoleDevice) {

			int i, c = IOEVENTFLAGS_GETCHARS(event.flags);
			jsiSetBusy(BUSY_INTERACTIVE, true);
			//jsiConsolePrint("in consoleDevice\n");
			//jsiConsolePrintf(" characters = %d\n",c);
			for (i=0;i<c;i++){
				//jsiConsolePrintf(" ch1 = %c  %d\n",event.data.chars[i],event.data.chars[i]);
				jsiHandleChar(event.data.chars[i]);
				//jsiConsolePrintf("ok\n");
				//find = 10;
			}
			jsiSetBusy(BUSY_INTERACTIVE, false);
       ...
       console device. It slows us down and just causes pain. */
			...
			while (jsvObjectIteratorHasValue(&it)) {
				...
				}  //end for if (jshIsEventForPin(&event, pin))
			...
			jsvObjectIteratorFree(&it);
			jsvUnLock(watchArrayPtr);
		}  //end for if (DEVICE_IS_EXTI(eventType))
	}  //end for while (maxEvents-- && jshPopIOEvent(&event))
	...
	while (jsvObjectIteratorHasValue(&it)) {
		...
		int timeUntilNext = timerTime - timePassed;
		//jsiConsolePrintf("timeUntilNext1 = %d\n",timeUntilNext);
		if (timeUntilNext < minTimeUntilNext)
			minTimeUntilNext = timeUntilNext;
		//jsiConsolePrintf("timeUntilNext<=0 = %d\n",timeUntilNext<=0);
		if (timeUntilNext<=0) {
			// we're now doing work
			//jsiConsolePrintf("doing now!\n");
			jsiSetBusy(BUSY_INTERACTIVE, true);
			wasBusy = true;
			JsVar *timerCallback = jsvObjectGetChild(timerPtr, "callback", 0);
			//jsiConsolePrintf("callback = ",timerCallback);
			...
			if (exec) {
				//jsiConsolePrintf("doing callback!\n");
	...
	if (todo) {
		...
		if (todo & TODO_RESET) {
			...
			jshReset();
			jsvInit();
			jsiConsolePrint("to do in reset.\n");
